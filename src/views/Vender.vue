<template>
  <div class="container">
   ...
   🔐{{T_mfmg.mtx[svl()][0].vl}}: <!--Multigarantía Ventas--> 
            ...
            {{T_mfmg.mtx[svl()][3].vl}}: ₿ <span style="color:DodgerBlue;">{{saldomfmg.toFixed(8)}}</span> 
            {{T_mfmg.mtx[svl()][4].vl}}: ₿ <span style="color:Red;">{{saldoresptxs.toFixed(8)}}</span> 
            {{T_mfmg.mtx[svl()][5].vl}}: ₿ <span style="color:Green;">{{round8(saldomfmg-saldoresptxs).toFixed(8)}}</span>
    ...
    <form id="formato" @submit.prevent="agregarVenta()" v-if="!editar">
        ...
        <popper
              ...
              <a  slot="reference"
                      ...
                      @click="fillcadenared()"
              >
              <b v-if="compra.CADENARED === '₿'"><i><font color="Black">🔗</font></i></b>
              <b v-if="compra.CADENARED === '⚡'"><font color="Darkorange">⚡</font></b>
              </a> 
        </popper>      
        ...
        />
        <b-button class="btn-success smy-1 smx-1" style="margin-bottom:15px;margin-top:5px;" type="submit">
          <img
            align="bottom"
            src="https://www.contobit.com/imagens/manita.png"
            width="25px"
  ...
  <div class="container_editar">
    ...
    <form id="formato" @submit.prevent="editarVenta(ofertaEditar)" v-if="editar">
      ...
      <input
        type="hidden"
        class="form-control my-2"
        placeholder="Cadena Bitcoin o Red lightning"
    ...
    <table class="table table-hover">
      <thead>
        <tr>
          <th scope="col"><b><i>🔗</i></b><br /><font color="Darkorange"> ⚡</font></th>
      ...
      <tbody>
        <tr v-for="(item, index) in comprar" :key="index">
          ...
          <td v-if="item.MONEDAOFERTA === 'USD'" :data-th="tituls.matrix[svl()][5].value">
            <font
              color="darkGreen"
            >{{(selectedbtc-(selectedbtc*item.PRECIOOFERTA)/-100) | currencydecimal}}</font>
            <br />
            <font
          ...
          <td v-else-if="item.MONEDAOFERTA === 'VES'" :data-th="tituls.matrix[svl()][5].value">
            <font
              color="darkGreen"
            >{{(selectedbtcVES-(selectedbtcVES*item.PRECIOOFERTA)/-100) | currencydecimal}}</font>
            <br />
            <font
              v-if="item.PRECIOOFERTA > 0"
              color="darkGreen"
          ...
          <td v-else-if="item.MONEDAOFERTA === 'EUR'" :data-th="tituls.matrix[svl()][5].value">
            <font
              color="darkGreen"
            >{{(selectedbtcEUR-(selectedbtcEUR*item.PRECIOOFERTA)/-100) | currencydecimal}}</font>
            <br />
            <font
              v-if="item.PRECIOOFERTA > 0"
              color="darkGreen"
          ...
          <td v-else-if="item.MONEDAOFERTA === 'COP'" :data-th="tituls.matrix[svl()][5].value">
            <font
              color="darkGreen"
            >{{(selectedbtcCOP-(selectedbtcCOP*item.PRECIOOFERTA)/-100) | currencydecimal}}</font>
            <br />
            <font
              v-if="item.PRECIOOFERTA > 0"
              color="darkGreen"
          ...
          <td v-else-if="item.MONEDAOFERTA === 'CLP'" :data-th="tituls.matrix[svl()][5].value">
            <font
              color="darkGreen"
            >{{(selectedbtcCLP-(selectedbtcCLP*item.PRECIOOFERTA)/-100) | currencydecimal}}</font>
            <br />
            <font
              v-if="item.PRECIOOFERTA > 0"
              color="darkGreen"
          ...
          <td v-else-if="item.MONEDAOFERTA === 'BRL'" :data-th="tituls.matrix[svl()][5].value">
            <font
              color="darkGreen"
            >{{(selectedbtcBRL-(selectedbtcBRL*item.PRECIOOFERTA)/-100) | currencydecimal}}</font>
            <br />
            <font
              v-if="item.PRECIOOFERTA > 0"
              color="darkGreen"
          ...
          <td v-else-if="item.MONEDAOFERTA === 'MXN'" :data-th="tituls.matrix[svl()][5].value">
            <font
              color="darkGreen"
            >{{(selectedbtcMXN-(selectedbtcMXN*item.PRECIOOFERTA)/-100) | currencydecimal}}</font>
            <br />
            <font
              v-if="item.PRECIOOFERTA > 0"
              color="darkGreen"
          ...
          <td v-else-if="item.MONEDAOFERTA === 'ARS'" :data-th="tituls.matrix[svl()][5].value">
            <font
              color="darkGreen"
            >{{(selectedbtcARS-(selectedbtcARS*item.PRECIOOFERTA)/-100) | currencydecimal}}</font>
            <br />
            <font
              v-if="item.PRECIOOFERTA > 0"
              color="darkGreen"
          ...
          <td v-else-if="item.MONEDAOFERTA === 'PEN'" :data-th="tituls.matrix[svl()][5].value">
            <font
              color="darkGreen"
            >{{(selectedbtcPEN-(selectedbtcPEN*item.PRECIOOFERTA)/-100) | currencydecimal}}</font>
            <br />
            <font
              v-if="item.PRECIOOFERTA > 0"
              color="darkGreen"
          ...
          <td v-else-if="item.MONEDAOFERTA === 'RUB'" :data-th="tituls.matrix[svl()][5].value">
            <font
              color="darkGreen"
            >{{(selectedbtcRUB-(selectedbtcRUB*item.PRECIOOFERTA)/-100) | currencydecimal}}</font>
            <br />
            <font
              v-if="item.PRECIOOFERTA > 0"
              color="darkGreen"
          ...
          <td v-else-if="item.MONEDAOFERTA === 'INR'" :data-th="tituls.matrix[svl()][5].value">
            <font
              color="darkGreen"
            >{{(selectedbtcINR-(selectedbtcINR*item.PRECIOOFERTA)/-100) | currencydecimal}}</font>
            <br />
            <font
              v-if="item.PRECIOOFERTA > 0"
              color="darkGreen"
          ...
          <td v-else-if="item.MONEDAOFERTA === 'TRY'" :data-th="tituls.matrix[svl()][5].value">
            <font
              color="darkGreen"
            >{{(selectedbtcTRY-(selectedbtcTRY*item.PRECIOOFERTA)/-100) | currencydecimal}}</font>
            <br />
            <font
              v-if="item.PRECIOOFERTA > 0"
              color="darkGreen"
          ...
          <td v-else-if="item.MONEDAOFERTA === 'CNY'" :data-th="tituls.matrix[svl()][5].value">
            <font
              color="darkGreen"
            >{{(selectedbtcCNY-(selectedbtcCNY*item.PRECIOOFERTA)/-100) | currencydecimal}}</font>
            <br />
            <font
              v-if="item.PRECIOOFERTA > 0"
              color="darkGreen"
          ...
          <td v-if="item.MONEDAOFERTA === 'USD'" class="sm-col-2"  :data-th="list_ofers.mtr[svl()][5].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEABAJO*selectedbtc | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEABAJO | currencydec8}}</div>
            </font>
          </td>
          <!--{{item.LIMITEARRIBA}} -->
          <td v-else-if="item.MONEDAOFERTA === 'VES'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][5].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEABAJO*selectedbtcVES| currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEABAJO | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'EUR'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][5].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEABAJO*selectedbtcEUR | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEABAJO | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'COP'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][5].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEABAJO*selectedbtcCOP | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEABAJO | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'CLP'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][5].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEABAJO*selectedbtcCLP | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEABAJO | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'BRL'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][5].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEABAJO*selectedbtcBRL | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEABAJO | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'MXN'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][5].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEABAJO*selectedbtcMXN | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEABAJO | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'ARS'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][5].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEABAJO*selectedbtcARS | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEABAJO | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'PEN'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][5].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEABAJO*selectedbtcPEN | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEABAJO | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'RUB'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][5].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEABAJO*selectedbtcRUB | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEABAJO | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'INR'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][5].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEABAJO*selectedbtcINR | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEABAJO | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'TRY'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][5].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEABAJO*selectedbtcTRY | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEABAJO | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'IRR'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][5].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEABAJO*selectedbtcIRR | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEABAJO | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'CNY'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][5].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEABAJO*selectedbtcCNY | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEABAJO | currencydec8}}</div>
          ...
          <td v-if="item.MONEDAOFERTA === 'USD'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][6].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEARRIBA*selectedbtc | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
          ...
          <td v-else-if="item.MONEDAOFERTA === 'VES'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][6].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEARRIBA*selectedbtcVES| currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEARRIBA | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'EUR'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][6].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEARRIBA*selectedbtcEUR | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEARRIBA | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'COP'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][6].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEARRIBA*selectedbtcCOP | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEARRIBA | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'CLP'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][6].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEARRIBA*selectedbtcCLP | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEARRIBA | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'BRL'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][6].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEARRIBA*selectedbtcBRL | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEARRIBA | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'MXN'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][6].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEARRIBA*selectedbtcMXN | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEARRIBA | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'ARS'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][6].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEARRIBA*selectedbtcARS | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEARRIBA | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'PEN'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][6].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEARRIBA*selectedbtcPEN | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEARRIBA | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'RUB'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][6].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEARRIBA*selectedbtcRUB | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEARRIBA | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'INR'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][6].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEARRIBA*selectedbtcINR | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEARRIBA | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'TRY'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][6].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEARRIBA*selectedbtcTRY | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEARRIBA | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'IRR'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][6].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEARRIBA*selectedbtcIRR | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEARRIBA | currencydec8}}</div>
          ...
          <td v-else-if="item.MONEDAOFERTA === 'CNY'" class="sm-col-2" :data-th="list_ofers.mtr[svl()][6].value">
            <font size="2" color="MediumSeaGreen">
              {{item.MONEDAOFERTA}} {{item.LIMITEARRIBA*selectedbtcCNY | currencydecimal}}
              <br />
              <div
                class="sm-col-2"
              >₿ {{item.LIMITEARRIBA | currencydec8}}</div>
...
export default {
  ...
  data() {
    return {
      ...
      T_mfmg: {
       mtx: [
          [{ vl: 'Multigarantía Ventas'},{ vl: '<br>🔐Multigarantía Ventas:<br><br> Dirección Bitcoin Contrato Multifirma 2/2 (Vendedor/Arbitraje), base de garantía para cualquier venta de bitcoins en la plataforma, ya sea en 🔗cadena principal Bitcoin o en ⚡Red Lightning, armando al Arbitraje con firmas de Transacciones(Tx) en Respaldo para los intercambios que se cursen.<br> Una vez creada la multifirma, a más tardar, necesaria tan pronto apareca una solicitud de compra en firme, y que usted como  Vendedor acepte. Deberá enviar a la Dirección Multifirma Multigarantía, COMO MÍNIMO, la Cantidad de Bitcoins de la Venta aceptada, mas el 0.25 % por Arbitraje.<br>** LA CANTIDAD DE BITCOINS QUE ENVIE O MANTENGA EN LA DIRECCIÓN BITCOIN MULTIFIRMA MULTIGARANTÍA ES REEMBOLSABLE EN  CUALQUIER MOMENTO  QUE USTED DECIDA (EN EL PERFIL DE USUARIO DE SU DIRECCIÓN BITCOIN), <strong>SIEMPRE Y CUANDO NO ESTE RESPALDANDO VENTA(s) EN CURSO</strong>. ADEMÁS, COMO SALVAGUARDA DE LIBERACIÓN DE SU GARANTÍA, TAMBIÉN, DISPONE EN EL PERFIL DE USUARIO DE UNA TX DE RETORNO ABIERTA Y FIRMADA POR EL ARBITRAJE, ASEGURANDO EL RETORNO DE LOS BITCOINS EN GARANTÍA AL VENCIMIENTO QUE DETERMINE ** <br>><font color=red><strong>RECOMENDACIONES Y CONSIDERACIONES. Si esta en sus posibilidades y desea agilizar al máximo, evitando a los compradores los tiempos de espera de 1 hora(6 confimaciones) para sus ventas(intercambios), y disponer de anuncios y Ventas de tipo INMEDIATAS a tiempo real</strong>, o bien si usted fuera vendedor habitual de Bitcoins en la plataforma:</font><br> 1.-* Hacer la mínima cantidad posible de envios a la Dirección Multifirma Multigarantía para evitar gastos de Minería Bitcoin. <br> 2.-* Mantener, como mínimo, un saldo en la Dirección Multifirma Multigarantía acorde a los límites máximos de Bitcoins publicados en sus Oferatas de Venta. Más aún si desea realizar ventas simultáneas en tiempo real, multiplicado por la cantidad de de ventas simultáneas que  pueda garantizar y atender al mismo tiempo.<br> 3.-* Las Ventas en ⚡Red Lightning requieren de la Tx de Respaldo congelada hasta la finalización de la entrega de bitcoins al Comprador via red Lightning.<br> 4.-* Las Ventas en 🔗cadena principal Bitcoin usarán la Tx de Respaldo para la Entrega de bitcoins al Comprador.<br><br>'},{ vl: ' Crear Multifirma de 🔐Multigarantía Ventas'},{ vl: 'Saldo'},{ vl: 'Respaldos'},{ vl: 'Disponible'}],

          [{ vl: 'Multi-warranty Sales'},{ vl: '<br> 🔐Multi-guarantee Sales: <br> <br> Bitcoin Address Multisignature Contract 2/2 (Seller / Arbitrage), guarantee basis for any sale of bitcoins on the platform, either in 🔗Bitcoin main chain or in ⚡Lightning Network , arming the Arbitration with signatures of Transactions (Tx) in Support for the exchanges that are carried out. <br> Once the multi-signature is created, at the latest, necessary as soon as a firm purchase request appears, and that you as the Seller accept . You must send to the Multi-Guarantee Multi-Signature Address, AT A MINIMUM, the Amount of Bitcoins from the accepted Sale, plus 0.25% per Arbitration. <br> ** THE AMOUNT OF BITCOINS THAT YOU SEND OR KEEP IN THE MULTI-SIGN MULTI-GUARANTEE BITCOIN ADDRESS IS REFUNDABLE IN ANY WHEN YOU DECIDE (IN THE USER PROFILE OF YOUR BITCOIN ADDRESS), <strong> ALWAYS AND WHEN YOU ARE NOT SUPPORTING CURRENT SALE (s) </strong>. IN ADDITION, AS A SAFEGUARD FOR RELEASE OF ITS WARRANTY, IT ALSO HAS IN THE USER PROFILE AN OPEN RETURN TX SIGNED BY THE ARBITRATION, ENSURING THE RETURN OF THE BITCOINS IN GUARANTEE AT THE EXPIRATION DETERMINATION ** <br>> <font color = red> <strong> RECOMMENDATIONS AND CONSIDERATIONS. If you are within your means and you want to streamline as much as possible, avoiding buyers waiting times of 1 hour (6 confirmations) for their sales (exchanges), and have IMMEDIATE ads and sales in real time </strong>, Or if you were a regular seller of Bitcoins on the platform: </font> <br> 1 .- * Make the minimum possible amount of shipments to the Multi-Signature Multi-Guarantee Address to avoid Bitcoin Mining expenses. <br> 2 .- * Maintain, as a minimum, a balance in the Multi-signature Multi-Guarantee Address according to the maximum limits of Bitcoins published in their Sales Offerings. Even more so if you want to make simultaneous sales in real time, multiplied by the number of simultaneous sales that you can guarantee and attend to at the same time. <br> 3 .- * Sales in ⚡Lightning network require the Backup Tx frozen up to completion of the delivery of bitcoins to the Buyer via Lightning network. <br> 4 .- * Sales in the main Bitcoin blockchain will use the Backup Tx for the Delivery of bitcoins to the Buyer. <br> <br>'},{ vl: ' Create Multi-signature of 🔐Multi-warranty Sales'},{ vl: 'Balance'},{ vl: 'Endorsements'},{ vl: 'Available'}],

          [{ vl: 'Multigarantia Vendes'},{ vl: '<br>🔐Multigarantia Vendes: <br><br> Adreça Bitcoin Contracte Multisignatura 2/2 (Venedor/Arbitratge), base de garantia per a qualsevol venda de bitcoins a la plataforma, ja sigui a 🔗cadena principal Bitcoin o a la ⚡Xarxa Lightning , armant a l`Arbitratge amb firmes de Transaccions (Tx) a Respatller per als intercanvis que es cursin.<br> Un cop creada la multisignatura, com a molt tard, necessària tan aviat apareix una sol·licitud de compra en ferm, i que vostè com a Venedor accepti. Haurà d`enviar a la Direcció Multifirma Multigarantía, COM A MÍNIM, la Quantitat de Bitcoins de la Venda acceptada, més el 0.25 % per Arbitratge.<br>** LA QUANTITAT DE BITCOINS QUE ENVIEU O MANTIGUEU A L`ADREÇA BITCOIN MULTIFIRMA MULTIGARANTIA ÉS REEMBORSABLE EN QUALSEVOL MOMENT QUE VOSTÈ DECIDEIXI (EN EL PERFIL D`USUARI DE LA SEVA DIRECCIÓ BITCOIN), <strong>SEMPRE I QUAN NO ESTIGUIN RESPALDANT VENDA(s) EN CURS</strong>. A MÉS, COM SALVAGUARDA D`ALLIBERAMENT DE LA SEVA GARANTIA, TAMBÉ, DISPOSA EN EL PERFIL D`USUARI D`UNA TX DE RETORN OBERTA I SIGNADA PER L`ARBITRATGE, ASSEGURANT EL RETORN DELS BITCOINS EN GARANTIA AL VENCIMIENT QUE ESTABLEIXI ** <br>><font color=red><strong>RECOMANACIONS I CONSIDERACIONS. Si està en les seves possibilitats i desitja agilitzar al màxim, evitant als compradors els temps d`espera d`una hora (6 confimacions) per a les vendes (intercanvis), i disposar d`anuncis i vendes de tipus IMMEDIATES a temps real</strong>, o bé si fos venedor habitual de Bitcoins a la plataforma:</font><br> 1.-* Fer la mínima quantitat possible d`enviaments a la Direcció Multifirma Multigarantía per evitar despeses de Mineria Bitcoin. <br> 2.-* Mantenir, com a mínim, un saldo a la Direcció Multifirma Multigarantia acord als límits màxims de Bitcoins publicats a les seves Oferates de Venda. Més encara si desitja realitzar vendes simultànies en temps real, multiplicat per la quantitat de vendes simultànies que pugui garantir i atendre al mateix temps.<br> 3.-* Les Vendes a la ⚡Xarxa Lightning requereixen de la Tx de Respatller congelada fins a la finalització del lliurament de bitcoins al Comprador via xarxa Lightning.<br> 4.-* Les Vendes a 🔗cadena principal Bitcoin faran servir la Tx de Respatller per a la Lliurament de bitcoins al Comprador.<br><br>'},{ vl: ' Crear Multifirma de 🔐Multigarantia Vendes'},{ vl: 'Saldo'},{ vl: 'Respatllers'},{ vl: 'Disponible'}],

          [{ vl: 'Vendas Multi-garantia'},{ vl: '<br> 🔐Venda Multi-garantia: <br> <br> Bitcoin Endereço Multi-assinatura Contrato 2/2 (Vendedor / Arbitragem), base de garantia para qualquer venda de bitcoins na plataforma, seja na 🔗Cadeia principal Bitcoin ou em ⚡ Red Lightning, armando a Arbitragem com Assinaturas de Transações (Tx) em Apoio às trocas que se realizam. <br> Uma vez criada a multi-assinatura, o mais tardar, necessária assim que apareça um pedido firme de compra, e que você como o vendedor aceita. Você deve enviar para o Endereço de Multi-Garantia de Multi-Assinatura, NO MÍNIMO, a Quantidade de Bitcoins da Venda aceita, mais 0,25% por Arbitragem. <br> ** A QUANTIA DE BITCOINS QUE VOCÊ ENVIE OU MANTENHA NO ENDEREÇO ​​MULTI -GARANTIA BITCOIN MULTI-GARANTIA É REEMBOLSÁVEL EM QUALQUER QUANDO VOCÊ DECIDIR (NO PERFIL DE USUÁRIO DO SEU ENDEREÇO ​​BITCOIN), <strong> SEMPRE E QUANDO VOCÊ NÃO ESTIVER APOIANDO VENDA (s) ATUAL (s) </strong>. ALÉM DISSO, COMO SALVAGUARDA PARA LIBERAÇÃO DE SUA GARANTIA, TAMBÉM TEM NO PERFIL DO USUÁRIO UM TX DE RETORNO ABERTO ASSINADO PELA ARBITRAGEM, GARANTIDO O RETORNO DAS BITCOINS GARANTIDAS NA DETERMINAÇÃO DE VENCIMENTO ** <br>> <font color = vermelho> <strong> RECOMENDAÇÕES E CONSIDERAÇÕES. Se você estiver ao seu alcance e quiser agilizar ao máximo, evitando que os compradores esperem de 1 hora (6 confirmações) para suas vendas (trocas), e tenham IMEDIATOS anúncios e vendas em tempo real </strong>, Ou se você era um vendedor regular de Bitcoins na plataforma: </font> <br> 1 .- * Faça a quantidade mínima possível de remessas para o Endereço de Multi-Garantia de Multi-Assinatura para evitar despesas de Mineração de Bitcoin. <br> 2 .- * Manter, no mínimo, um saldo no Endereço Multi-Assinatura Multi-Garantias de acordo com os limites máximos de Bitcoins publicados em suas Ofertas de Venda. Ainda mais se você quiser fazer vendas simultâneas em tempo real, multiplicado pelo número de vendas simultâneas que você pode garantir e atender ao mesmo tempo. <br> 3 .- * As vendas em ⚡Red Lightning requerem o Backup Tx congelado até a conclusão da entrega de bitcoins ao comprador por meio da rede Lightning. <br> 4 .- * As vendas na cadeia principal de bitcoins usarão o Backup Tx para a entrega de bitcoins ao comprador. <br> <br>'},{ vl: ' Criar Multi-assinatura de 🔐 Vendas Multi-garantia '},{ vl: 'Equilíbrio'},{ vl: 'Endossos'},{ vl: 'Disponível'}],                                             
            ],
      },
      Textos_qrcode: {
       vars: [
      ...
      titul: {
       matrix: [
                [{ value: 'Vender Bitcoins' }, { value: '<br><b>Vender Bitcoins:</b><br> Para agregar una oferta de venta nueva, debe rellenar o seleccionar en TODAS las casillas de la linea que sigue y luego pulsar el botón de la manita vender al final de la misma.<br><br><b><i>🔗</i></b> · <font color=Darkorange><b> ⚡</b></font>:<br> Pulse en el botón para invertir la selección según desee ofertar en <b><i>🔗</i></b> Cadena de bloques Bitcoin o en <font color=Darkorange><b> ⚡</b></font> Red de rayos. A través de la cual realizar el proceso Tx de compra-venta.<br><br>Detallles de la oferta:<br> Introduzca texto o emojis a conveniencia hasta máximo 65 caracteres. Ej. Conectado en las mañanas de lunes a viernes <br><br>Medio de cobro:<br> Seleccione en el menú desplegable de la casilla, dentro del grupo escogido(explore pulsando ▶ a la izquiera de cada opción para visualizar submenús que contiene cada una), el medio de cobro aceptado para la oferta. <br><br>Precio % ₿:<br> Introduzca porcentaje negativo o positivo dependiendo si desea vender por debajo  o por encima del precio de mercado. Ej. -2.13 <br><br>Moneda de cobro:<br>Pulse  dentro de la casilla sobre las siglas de moneda para abrir el selector y SELECCIONE la moneda de cobro aceptada. Ej. EUR  <br><br>Venta Mínima ₿:<br>Introcuzca en la casilla el valor mínimo aceptado para una venta expresado en bitcoins hasta 8 decimales . Ej. 0.00123456  <br><br>Venta Máxima ₿:<br>Introcuzca en la casilla el valor máximo aceptado para una venta expresado en bitcoins hasta 8 decimales . Ej. 0.03123456  <br><br>Posteriormente puede modificar o eliminar cualquier oferta con los botones destinados. Al terminar revise también su publicación en Comprar del menú principal.<br><br>' }, { value: ' ➕ Agregar nueva Oferta' }, { value: ' Editar Mi Oferta de Venta' }],
                [{ value: 'Sell Bitcoins' }, { value: '<br><b>Sell Bitcoins:</b><br> To add a new sale offer, you must fill in or select in ALL the boxes of the line that follows and then press the button of the hand sell at the end of it. <br><br><b><i>🔗</i></b> · <font color=Darkorange><b> ⚡</b></font>:<br> Click the button to invert the selection as you wish to bid on <b><i>🔗</i></b> Bitcoin blockchain or in <font color=Darkorange><b> ⚡</b></font> Lightning network. Through which to carry out the Tx purchase-sale process.<br><br> Details of the offer: <br> Enter Text or emojis at your convenience up to a maximum of 65 characters. Eg Connected in the mornings from Monday to Friday <br> <br> Payment method: <br> Select from the drop-down menu of the box, within the chosen group (explore by pressing ▶ ️ to the left of each option to display submenus that contains each one), the means of payment accepted for the offer. <br> <br> Price% ₿: <br> Enter a negative or positive percentage depending on whether you want to sell below or above the market price. Ex. -2.13 <br> <br> Collection currency: <br> Click inside the box on the currency initials to open the selector and SELECT the accepted collection currency. Eg EUR <br> <br> Minimum Sale ₿: <br> Enter in the box the minimum value accepted for a sale expressed in bitcoins up to 8 decimal places. Ex. 0.00123456 <br> <br> Maximum Sale ₿: <br> Enter in the box the maximum value accepted for a sale expressed in bitcoins up to 8 decimal places. Ex. 0.03123456 <br> <br> You can later modify or delete any offer with the buttons provided. When finished, also check your publication in Buy from the main menu.<br><br>' }, { value: ' ➕ Add new Offer' }, { value: ' Edit My Sale Offer' }],
                [{ value: 'Vendre Bitcoins' }, { value: '<br><b>Vendre Bitcoins:</b><br> Per afegir una oferta de venda nova, ha d`omplir o seleccionar a TOTES les caselles de la línia que segueix i després prémer el botó de la maneta vendre a la fi de la mateixa.<br><br><b><i>🔗</i></b> · <font color=Darkorange><b> ⚡</b></font>:<br> Feu clic al botó per invertir la selecció segons desitgi oferir en <b><i>🔗</i></b> Cadena de blocs Bitcoin o en <font color=Darkorange><b> ⚡</b></font> Xarxa de llamps. A través de la qual realitzar el procés Tx de compravenda.<br><br> Detalls de l`oferta:<br> Introdueixi texte o emojis a conveniència fins màxim 65 caràcters. Ex. Conectat als matins de dilluns a divendres. <br><br> Mitjà de cobrament: <br> Seleccioneu al menú desplegable de la casella, dins el grup escollit (explori prement ▶ ️ a l` esquerra de cada opció per a visualitzar els submenús que conté cadascuna), el mitjà de cobrament acceptat per l` oferta. <br><br> Preu% ₿: <br> Introdueixi percentatge negatiu o positiu depenent si desitja vendre per sota o per sobre del preu de mercat. Ex. -2.13 <br><br> Moneda de cobrament: <br> Premi dins de la casella sobre les sigles de moneda per obrir el selector i SELECCIONI la divisa de cobrament acceptada. Ex. EUR <br><br> Venda Mínima ₿: <br> Introdueixi a la casella el valor mínim acceptat per a una venda expressat en bitcoins fins a 8 decimals. Ex. 0.00123456 <br><br> Venda Màxima ₿: <br><br> Introdueixi a la casella el valor màxim acceptat per a una venda expressat en bitcoins fins a 8 decimals. Ex. 0.03123456 <br><br> Posteriorment pot modificar o eliminar qualsevol oferta amb els botons destinats. Al finalitzar revisi també la publicació dins Comprar del menú principal.<br><br>' }, { value: ' ➕ Afegeix una nova oferta' }, { value: ' Editar La meva Oferta de Venda' }],
                [{ value: 'Vender Bitcoins' }, { value: '<br><b>Vender Bitcoins:</b><br> Para adicionar uma nova oferta de venda, deve preencher ou seleccionar TODOS os quadrados da linha que se segue e premir o botão da mão vender no final da mesma. <br><br><b><i>🔗</i></b> · <font color=Darkorange><b> ⚡</b></font>:<br> Clique no botão para inverter a seleção conforme deseja licitar <b><i>🔗</i></b> Corrente de bloco Bitcoin ou em <font color=Darkorange><b> ⚡</b></font> Rede relâmpago. Através do qual realizar o processo de compra-venda de Tx.<br><br> Detalhes da oferta: <br> Entrar Texto ou emojis de acordo com sua conveniência com até 65 caracteres. Ex: conectado de manhã de segunda a sexta-feira <br> <br> Método de pagamento: <br> Selecione no menu suspenso da caixa, dentro do grupo escolhido (explore pressionando ▶ ️ à esquerda de cada opção para exibir os submenus que contém cada um), o meio de pagamento aceito para a oferta. <br> <br> Preço% ₿: <br> Insira uma porcentagem negativa ou positiva, dependendo se deseja vender abaixo ou acima do preço de mercado. Ex. -2,13 <br> <br> Moeda de cobrança: <br> Clique dentro da caixa nas iniciais da moeda para abrir o seletor e SELECIONE a moeda de cobrança aceita. Ex: EUR <br> <br> Venda Mínima ₿: <br> Insira na caixa o valor mínimo aceito para uma venda expresso em bitcoins até 8 casas decimais. Ex. 0,00123456 <br> <br> Venda Máxima ₿: <br> Insira na caixa o valor máximo aceito para uma venda expresso em bitcoins até 8 casas decimais. Ex. 0.03123456 <br> <br> Posteriormente, você pode modificar ou excluir qualquer oferta com os botões fornecidos. Quando terminar, verifique também a sua publicação em Comprar no menu principal.<br><br>' }, { value: ' ➕ Adicionar nova oferta' }, { value: ' Editar Minha Oferta de Venda' }],   
               ],            
      }, 
      selch: {
       matrix: [
                [{ value: 'Click selecciona bitcoin o lightning' }],
                [{ value: 'Click select bitcoin or lightning' }],
                [{ value: 'Click selecciona bitcoin o lightning' }],
                [{ value: 'Clique em selecionar bitcoin ou lightning' }],   
               ]
      },
      detalls: {
       mtr: [
                [{ value: 'Detalles Oferta' },{ value: '_🔎 Medio de cobro' },{ value: 'Precio (+/-)%₿' },{ value: 'Vta.Mínima BTC' },{ value: 'Vta.Máxima BTC' }],
                [{ value: 'Offer details' },{ value: '_🔎 Payment method' },{ value: 'Price (+/-)%₿' },{ value: 'Min.Sale BTC' },{ value: 'Max.Sale BTC' }],
                [{ value: 'Detalls de l`oferta' },{ value: '_🔎 Mitjà de cobrament' },{ value: 'Preu (+/-)%₿' },{ value: 'Vda.Mínima BTC' },{ value: 'Vda.Màxima BTC' }],
                [{ value: 'Detalhes da oferta' },{ value: '_🔎 Forma de pagamento' },{ value: 'Preço (+/-)%₿' },{ value: 'Vda.Mínima BTC' },{ value: 'Vda.máxima BTC' }],   
               ]
      },
      etiq: {
       mtr: [
                [{ value: 'Editar detalles a publicar en la oferta' },{ value: 'Seleccione Medio de cobro para la oferta' },{ value: 'Editar Precio de la oferta (+/-)% sobre la cotización de bitcoin' },{ value: 'Seleccione Moneda de cobro para la  oferta' },{ value: 'Editar Cantidad Mínima de bitcoins para una venta' },{ value: 'Editar Cantidad Máxima de bitcoins para una venta' }],
                [{ value: 'Edit details to publish in the offer' },{ value: 'Select Payment method for the offer' },{ value: 'Edit Offer Price (+/-)% on bitcoin price' },{ value: 'Select Charge Currency for the offer' },{ value: 'Edit Minimum amount of bitcoins for a sale' },{ value: 'Edit Maximum amount of bitcoins for a sale' }],
                [{ value: 'Editar detalls a publicar a l`oferta' },{ value: 'Seleccioni Mitjà de cobrament per la oferta' },{ value: 'Edita Preu de l`oferta (+/-)% sobre la cotització de bitcoin' },{ value: 'Seleccioneu Moneda de cobrament per a l`oferta' },{ value: 'Editar Quantitat Mínima de bitcoins per a una venda' },{ value: 'Editar Quantitat Màxima de bitcoins per a una venda' }],
                [{ value: 'Edite os detalhes para publicar na oferta' },{ value: 'Selecione Forma de pagamento da oferta' },{ value: 'Editar preço da oferta (+/-)% sobre o preço do bitcoin' },{ value: 'Selecione a moeda de cobrança para a oferta' },{ value: 'Editar Quantidade mínima de bitcoins para uma venda' },{ value: 'Editar Quantidade Máxima de bitcoins para uma venda' }],   
               ]
      },
      list_ofers: {
       mtr: [
      ...
      f_fillcaden: {
       mtx: [
                [{ vl: 'Oferta para venta en ⚡ Red Lightning, asegurese de disponer de cartera con saldo suficiente y canales abiertos en linea' }, { vl: 'Oferta para venta en 🔗 cadena de bloques Bitcoin, asegurese de disponer de algún saldo en su dirección bitcoin para mantener la publicación' }],

                [{ vl: 'Offer for sale in ⚡ Lightning network, make sure you have a portfolio with sufficient balance and open online channels' }, { vl: 'Offer for sale on 🔗 Bitcoin blockchain, make sure you have some balance in your bitcoin address to maintain the publication' }],

                [{ vl: 'Oferta per a venda en ⚡ Xarxa Lightning, assegureu-vos disposar de cartera amb saldo suficient i canals oberts en línia' }, { vl: 'Oferta per a venda en 🔗 cadena de blocs Bitcoin, assegureu-vos disposar d`algun saldo a l`adreça bitcoin per mantenir la publicació' }],

                [{ vl: 'Oferta à venda na ⚡ Rede Lightning, certifique-se de ter um portfólio com saldo suficiente e canais online abertos.' },{ vl: 'Oferta à venda no 🔗 blockchain de Bitcoin, certifique-se de ter algum saldo em seu endereço de bitcoin para manter a publicação' }],                                               
               ]
      },
      f_checkpreu: {
       mtx: [
  ...
  methods: {
    ...
    fillcadenared_edit() { 
      if (this.ofertaEditar.CADENARED === '₿') {
       ...
       this.ofertaEditar.CADENARED = '₿';
       this.mensaje.texto = this.f_fillcaden.mtx[this.svl()][1].vl;    
       this.showAlert();        
      } 
    },
    fillcadenared() { 
